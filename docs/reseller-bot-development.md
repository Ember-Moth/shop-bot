...
| 上游生产适配器 | 待实现；当前 `build_upstream()` 仍返回 `StubUpstreamClient` |
...
**尚不能宣称已经可正式自动售卖：** 当前生产代码仍是模拟上游，真实扣款、KYC 放行和交付尚未验收。

真正阻止 Live 自动采购启用的项目是：适配器和采购状态尚未实现、真实资金规则尚未验证，以及缺少完整采购样本。现有信息足够开展这些工作。

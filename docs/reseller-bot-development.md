...
| 上游生产适配器 | 已实现（阶段 B）：`services/purchasing.py` 采购状态机 + `create_request`；配置 `upstream.provider: commbitz` 即启用 |
| 实际采购、钱包扣款、KYC 上传、真实交付 | 尚未联调 |
...
真正阻止 Live 自动采购启用的项目是：真实资金规则尚未验证、KYC 流程（阶段 C）尚未实现，以及缺少完整采购样本。适配器、采购记录与人工核对机制已就绪。

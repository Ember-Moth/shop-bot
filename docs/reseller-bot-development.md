...
| 实际采购、钱包扣款、真实交付 | 主链路已在生产环境用真实订单验证（截至 2026-09-24）；KYC 放行、实体卡物流等分支尚未在生产触发 |
| 上线验收 | 在售 SKU 已上线售卖；失败分支与对账项见 [生产验收记录](progress.md#生产验收记录) |
...
8. **通知可恢复**：货品先保存，只私信订单买家；单张 eSIM 发二维码图片，多张按最多 100 张/包的 ZIP 发送。发送中断可能导致同一内容补发，但不再次采购。
9. **敏感信息按需使用**：不记录令牌、API 密钥、证件内容、完整 LPA、PIN/PUK；KYC 和设备信息仅向有权访问的用户展示。
10. **恢复按任务执行**：收款和采购任务同事务保存；独立工作协程按到期时间推进采购与通知，不扫描全部历史订单。具体实现见 [架构](architecture.md#持久化任务调度)。
...
本店真实收款联调已完成：生产机器人、EPay 商户、公网 HTTPS 回调与在售 SKU 售价均已配置并跑通实单。创建请求仍必须按正式采购对待；新增 SKU 验收时明确测试数量和费用上限。
...
**已可正式自动售卖（在售 SKU）：** 截至 2026-09-24，真实付款、采购与交付主链路已在生产环境跑通。适配器、采购记录、人工核对机制与 KYC/用量流程（阶段 B/C）已就绪。

**仍以离线回归为唯一证据的部分：** 失败退款、结果不明的人工处置、KYC 放行、实体卡物流、扣款对账等分支尚未在生产触发；清单与勾选进度见 [生产验收记录](progress.md#生产验收记录)。未验收的业务类型保持关闭或人工处理状态。

...
- [x] 钱包加固（审计修复）：余额支付订单写占位 trade_no 阻断 EPay 收银台重复收款；`/adjust <用户ID> <±金额> [备注]` 人工调账（负余额拒绝、adjust 流水、私信用户）；充值回调核验与商品订单拉齐（交易号拒首尾空白、金额先过格式正则）；充值等金额状态非文本消息不再崩溃
...
钱包体系审计修复：余额支付订单写占位 trade_no（`BAL<订单号>`），余额支付后
EPay 收银台重复付款的回调因 trade_no 一致性校验被拒（422 告警可溯），堵上
双通道重复收款窗口；新增 /adjust 人工调账命令（条件 UPDATE 不允许负余额、
adjust 流水同事务、私信用户知情），履约失败的余额退款有了出口；充值回调核验
与商品订单 validate_payment 拉齐（交易号拒首尾空白、金额先过格式正则再精确
比较，拒绝科学计数法等非常规格式）；TopupFlow 金额输入限定文本消息并给非文本
兜底提示；「我的余额」未注册用户文案改为引导 /start 注册。每条发现均配回归测试。

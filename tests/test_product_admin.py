...
async def test_admin_price_publish_unpublish_and_audit(db, bot, monkeypatch):
    ...
    assert not (await db.products.get_product(1)).active
    assert (await db.products.get_product(1)).price_cents == 1995
    assert (await db.products.get_product(1)).name == "test"  # 名称不被目录同步覆盖
...
async def test_admin_rename_and_audit(db, bot):
    ...
    assert (await db.products.get_product(1)).name == "美国1GB·7天"
    events = await db.fetch_all("SELECT * FROM product_events")
    assert len(events) == 1
    ...
    with pytest.raises(ValueError, match="名称"):
        await db.products.configure_product(1, name="   ")
    # 名称进入买家可见目录，多行消息注入的换行必须拒绝（审计 P2-1）
    with pytest.raises(ValueError, match="控制字符"):
        await db.products.configure_product(1, name="美国\n1GB")
...
async def test_admin_describe_and_audit(db, bot):
    ...
    assert (await db.products.get_product(1)).description == "美国TMO 100条短信+50分钟通话-30天"
    events = await db.fetch_all("SELECT * FROM product_events")
    assert len(events) == 1
    ...
    with pytest.raises(ValueError, match="描述"):
        await db.products.configure_product(1, description="x" * 501)
    with pytest.raises(ValueError, match="控制字符"):
        await db.products.configure_product(1, description="多行\n描述")


async def test_live_publish_requires_mapping(db, product):
    await db.products.configure_product(product.id, active=False)
    with pytest.raises(ValueError, match="SKU"):
        await db.products.configure_product(product.id, active=True, require_upstream=True)
    assert not (await db.products.get_product(product.id)).active
...
async def test_product_audit_and_update_roll_back_together(db, product):
    ...
    with pytest.raises(sqlite3.IntegrityError, match="audit failure"):
        await db.products.configure_product(product.id, price_cents=1000, actor_id=700)
    assert (await db.products.get_product(product.id)).price_cents == product.price_cents


async def test_whois_searches_by_telegram_id_username_and_display_name(db, bot):
    await db.users.upsert_user(111, "alice", "爱丽丝")
    await db.users.upsert_user(222, "bob", None)
    msg = menu_message_of(bot)

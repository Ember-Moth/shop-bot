...
async def test_duplicate_payment_and_admin_confirmation_do_not_wait_for_order_lock(db, user, product, epay, bot):
    ...
    async with callback_server(db, epay, bot) as client:
        ...
        assert await db.wallet.get_balance(user.id, product.currency) == 0
        item = await db.work.claim_work("purchase")
        assert item.entity_id == order.id
        assert await db.work.claim_work("purchase") is None
...
async def test_payment_and_task_intent_roll_back_together(db, user, product, source):
    ...
    with pytest.raises(aiosqlite.IntegrityError, match="queue unavailable"):
        if source == "epay":
            ...
            await db.wallet.complete_topup(topup.id, "TRADE")
        else:
            await db.wallet.adjust_balance(user.id, 100, "adjust")
    ...
    assert (await db.wallet.get_topup(topup.id)).status == "pending"
...
async def test_zip_upload_does_not_block_new_procurement(db, bot, monkeypatch):
    ...
    try:
        async with asyncio.timeout(4):
            await started.wait()
            other = await db.users.upsert_user(43, "other")
            order = await orders.create_order(db, other.id, product, 1)
            await orders.mark_paid(db, DemoPurchaser(), order.id)
            await second_delivered.wait()
        assert not release.is_set()
        assert (await db.orders.get_order(first_id)).notification_pending
    finally:
        release.set()
        worker.cancel()
...
async def test_procurement_concurrency_is_bounded_and_slow_supplier_does_not_block_notifications(
    ...
    try:
        async with asyncio.timeout(4):
            await entered.wait()
            topup = await db.wallet.create_topup(user.id, 100)
...
async def test_cancelled_submission_never_recreates_upstream_purchase(db, user, product):
    ...
    assert (await db.purchases.get_purchase_by_order(order.id)).state == PurchaseState.SUBMISSION_UNKNOWN
    assert await db.work.claim_work("purchase") is None
...
async def test_wallet_failure_survives_restart_and_respects_retry_after(tmp_path, bot, monkeypatch, queue_clock):
    ...
    topup = await db.wallet.create_topup(user.id, 999)
    await db.wallet.complete_topup(topup.id, "T1")
    ...
    try:
        ...
        assert not bot.session.sent
        queue_clock(59)
        assert await db.work.claim_work("wallet") is None
        queue_clock(2)
        await recover_once(db, DemoPurchaser(), bot)
        assert len(bot.session.sent) == 1
        await db.wallet.complete_topup(topup.id, "T1")
        ...
        assert len(bot.session.sent) == 1
        # 另一笔真实付款仍独立入账、独立通知。
        await db.wallet.complete_topup(topup.id, "T2")
        await recover_once(db, DemoPurchaser(), bot)
        assert len(bot.session.sent) == 2
        assert await db.wallet.get_balance(user.id, "CNY") == 1998
    finally:
        await db.close()
...
async def test_stale_completion_cannot_erase_new_work(db, user, product):
    ...
    await db.work.finish_work(item, done=True)
    replacement = await db.work.claim_work("purchase")
...
async def test_claimed_wallet_task_recovers_once_after_restart(tmp_path, bot):
    ...
    await db.connect()
    user = await db.users.upsert_user(42, "buyer")
    await db.wallet.adjust_balance(user.id, 250, "test")
    assert await db.work.claim_work("wallet") is not None
    assert await db.work.claim_work("wallet") is None
    await db.close()
    await db.connect()
    try:
